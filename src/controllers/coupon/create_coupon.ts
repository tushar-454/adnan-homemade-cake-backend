...
const createCoupon = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { code, type, discount, quantity, startAt, expireAt } = req.body as CreateCouponInput;
    const coupon = await createCouponService({ code, type, discount, quantity, startAt, expireAt });
